...
export class GolfRoundComponent implements OnInit {
  ...
  getGolfRounds() {
    this._golfRoundService.getGolfRounds().subscribe(data => {
      ...
      this.golfRoundViews.sortingDataAccessor = (item, property) => {
        switch (property) {
          ...
          case "teeName":
            return item.course.teeName;
          case "par":
            return item.course.par;
          case "scratchRating":
            return item.course.scratchRating;
          case "slope":
            return item.course.slope;
        }
      };
